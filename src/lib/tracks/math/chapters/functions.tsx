...
export function AlgebraContent({ t }: { t: TrackTranslations }) {
  return (
    <Article>
      ...
      <p>
        {tx(t, "mAlg_fnBody",
          "A function is a rule that turns each input into exactly one output: f(x) = x² turns 3 into 9. The set of allowed inputs is the domain (√x only accepts x ≥ 0 among reals), the set of outputs it can produce is the range (x² never produces negatives). Its graph is the set of all points (x, f(x)), drawn as a curve. Everyday life is full of functions: the postage as a function of the weight, the height of a thrown ball as a function of time, the temperature as a function of the hour. Thinking of them as curves you can see and reshape makes them much easier to understand.")}
      </p>
      <Equation label={tx(t, "mFn_eqNotation", "Function notation")}
        where={[
          [r`f`, tx(t, "mFn_wF", "the name of the function, the rule itself. Other letters work too: g, h, or descriptive names like cost")],
      ...
      <p>
        {tx(t, "mFn_machineBody",
          "A good mental picture is a machine: an input goes in, the rule is applied, one output comes out. The same input always gives the same output. A vending machine is a good example: pressing B3 always gives the same snack. The only requirement is \"exactly one output per input\"; two different inputs may share an output (x² gives 4 for both 2 and −2).")}
      </p>
      <LessonTable
        ...
        rows={[
          ...
          [tx(t, "mFn_r2", "table"), "x: 0, 1, 2 → f: 1, 3, 5", tx(t, "mFn_r2g", "measured data, timetables, price lists")],
          ...
          [tx(t, "mFn_r4", "words"), tx(t, "mFn_r4e", "\"double it and add one\""), tx(t, "mFn_r4g", "explaining the rule, and translating word problems")],
        ]}
      />
      ...
      <p>
        {tx(t, "mFn_pieceBody",
          "Many useful functions use different rules on different parts of the domain. The absolute value is the classic: |x| is x when x ≥ 0 and −x when x < 0. Tax brackets, postage by weight band and parking fees (\"free for the first 2 hours, then 3 per hour, never more than 24 a day\") are all piecewise. To evaluate one, first find which piece the input falls in, then use only that piece's rule. Check the joins: if the pieces do not meet where they switch, the graph jumps there.")}
      </p>
      <Equation label={tx(t, "mFn_eqFalloff", "A parking fee")}
        where={[
          [r`d`, tx(t, "mFn_wD", "the parking time, in hours")],
          [r`0`, tx(t, "mFn_w100", "the first 2 hours are free")],
          [r`3\,(d - 2)`, tx(t, "mFn_wLin", "3 per hour for every hour beyond the first 2; d − 2 counts the paid hours")],
          [r`24`, tx(t, "mFn_wCap", "the daily maximum, reached when 3(d − 2) = 24, that is d = 10")],
        ]}
      ...
      <p>
        {tx(t, "mFn_compBody",
          "Feeding one function's output into another is composition, written (f ∘ g)(x) = f(g(x)): apply g first, then f. The order matters: squaring then adding 1 is not adding 1 then squaring. Converting a price from euros to dollars and then adding 10% tax is a composition of two functions. An inverse function f⁻¹ undoes f: f⁻¹(f(x)) = x. It only exists if f never sends two inputs to the same output (x² sends 2 and −2 to 4, so √ can only undo it for x ≥ 0). The graph of f⁻¹ is the graph of f mirrored across the diagonal line y = x, because inverting swaps the roles of input and output. You will see that mirror again in the exponents chapter, between exponentials and logarithms.")}
      </p>
      <Equation label={tx(t, "mAlg_eqInv", "Inverting a linear function")}
        where={[
          [r`y = m\,x + b`, tx(t, "mAlg_wFwd", "the forward function")],
          [r`x = (y - b) / m`, tx(t, "mAlg_wBack", "solving for x gives the inverse: undo + b, then undo × m")],
        ]}>
      ...
      <LessonTable
        ...
        rows={[
          ...
          [tx(t, "mFn_m5w", "ignoring the domain"), tx(t, "mFn_m5r", "exclude zeros of denominators, negatives under √ and in log"), tx(t, "mFn_m5", "outside the domain the expression has no value at all")],
        ]}
      />
