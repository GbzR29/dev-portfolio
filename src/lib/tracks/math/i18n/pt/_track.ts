// PT chapter titles and section names for the math track (see math/index.tsx).

const track = {
  titles: {
    "number-line": "Números e a reta numérica",
    "order-of-operations": "Ordem das operações",
    fractions: "Frações e decimais",
    ratios: "Razões, proporção e porcentagem",
    divisibility: "Divisibilidade, primos, MDC e MMC",
    powers: "Potências e raízes",
    bases: "Bases numéricas",
    expressions: "Expressões",
    "linear-equations": "Equações do primeiro grau",
    inequalities: "Inequações",
    algebra: "Funções e gráficos",
    "linear-systems": "Sistemas lineares",
    quadratics: "Equações do segundo grau",
    polynomials: "Polinômios",
    exponents: "Expoentes e logaritmos",
    sequences: "Sequências e séries",
    angles: "Pontos, retas e ângulos",
    triangles: "Triângulos e congruência",
    area: "Perímetro e área",
    pythagoras: "O teorema de Pitágoras",
    similarity: "Semelhança e escala",
    circle: "Círculos e π",
    volumes: "Volume e área de superfície",
    analytic: "Geometria analítica e cônicas",
    transformations: "Transformações",
    trig: "Trigonometria no triângulo retângulo",
    "unit-circle": "Radianos e o círculo unitário",
    "triangle-laws": "Leis dos senos e dos cossenos",
    identities: "Identidades e rotação",
    polar: "Coordenadas polares e atan2",
    waves: "Ondas e oscilação",
    vectors: "Vetores",
    dot: "O produto escalar",
    cross: "O produto vetorial",
    matrices: "Matrizes e transformações lineares",
    determinant: "O determinante",
    inverse: "Matriz inversa e sistemas lineares",
    eigen: "Autovalores e autovetores",
    complex: "Números complexos",
    quaternions: "Quatérnios e rotação 3D",
    limits: "Limites e continuidade",
    derivatives: "A derivada",
    "derivative-rules": "Regras de derivação",
    "derivative-uses": "Aplicações da derivada",
    integrals: "Integrais: área por faixas",
    ftc: "O teorema fundamental",
  } as Record<string, string>,
  sections: {
    Arithmetic: "Aritmética",
    Algebra: "Álgebra",
    Geometry: "Geometria",
    Trigonometry: "Trigonometria",
    "Linear Algebra": "Álgebra linear",
    Calculus: "Cálculo",
  } as Record<string, string>,
};

export default track;
