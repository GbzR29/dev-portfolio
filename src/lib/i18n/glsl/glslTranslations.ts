...
export const glslTranslations = {
  en: {
    ...
    glsl03_uvVisTitle:    "Seeing UV, one fragment at a time",
    glsl03_uvVisBody:     "The widget below runs a real fragment shader. Drop the resolution to a few fragments and hover them: every square is one call to main(), with its own gl_FragCoord. Dividing it by uResolution gives the UV, and the shader turns that UV into a colour. Switch presets to see centering, fract() tiling, polar coordinates and time, or edit the code directly.",
    glsl03_uvVisTip:      "gl_FragCoord points at the centre of the fragment, so the bottom-left one is (0.5, 0.5), not (0, 0). Reading UV as a colour (red = u, green = v) is also how you debug a shader: when something looks wrong, output the value you doubt as a colour and check it.",
