...
export const openglTranslations = {
  en: {
    ...
    ch01_ndcInteractiveTip: "Drag any vertex on the canvas above, or click it to get an X/Y gizmo that moves it along a single axis. You can also type exact values in the coordinate fields. Notice how moving a point outside the [-1, 1] boundary clips it — the triangle edge disappears at the border. The CCW/CW indicator shows the winding order (covered in chapter 10).",
  ...
  pt: {
    // ── Interactive figures ──
    ch07_texFig: "Esses parâmetros ficam mais fáceis de escolher depois que você os vê. Cada pixel da figura abaixo é calculado do jeito que a GPU amostra uma textura — troque as abas para comparar modos de wrap, filtros de magnificação e mipmaps.",
    ch08_frustumBody: "Esses quatro números descrevem um volume chamado frustum de visão: uma pirâmide com a ponta cortada. Tudo que está dentro dele aparece na tela, tudo que está fora é recortado. Gire a figura abaixo e depois troque para ortográfica para ver a pirâmide virar uma caixa.",
    ch08la_homogFig: "Por que adicionar um componente torna a translação possível? É mais fácil ver uma dimensão abaixo: pontos 2D com uma terceira coordenada w. Avance a figura abaixo passo a passo.",
    ch08la_journeyIntro: "Agora veja as três acontecerem com um único vértice. Cada passo move a cena inteira para o próximo espaço, e a tabela acompanha as coordenadas desse mesmo vértice pelo caminho.",
    ch08la_vecFig: "Os dois produtos que você vai usar o tempo todo ficam mais fáceis de confiar depois que você os vê. O produto escalar é uma sombra; o produto vetorial é uma área em pé.",
    ch09_phongFig: "Os três termos vêm de poucos vetores no ponto da superfície: a normal N, a direção para a luz L, a direção para o olho V e a reflexão R. Arraste o sol e o olho abaixo e veja cada termo reagir.",
    figCam_hint: "arraste a figura para girar · role para dar zoom",
    figCam_resetView: "resetar vista",
    figFrus_title: "O frustum de visão — por fora e por dentro",
    figFrus_persp: "Perspectiva",
    figFrus_ortho: "Ortográfica",
    figFrus_camView: "O que a câmera vê (NDC)",
    figFrus_clipped: "recortado",
    figFrus_partial: "parcialmente recortado",
    figFrus_culled: "descartado",
    figFrus_depth: "Profundidade: z_ndc em função da distância",
    figFrus_depthPersp: "Metade de todos os valores de profundidade é gasta nos primeiros {pct}% do intervalo (até {d} unidades). Aproxime o near de 0 e veja isso encolher — é daí que vem o z-fighting.",
    figFrus_depthOrtho: "Na ortográfica a profundidade é linear: cada unidade de distância recebe a mesma fatia de precisão.",
    figFrus_moveB: "distância de B",
    figFrus_notePersp: "Afaste o B: ele encolhe, porque x e y são divididos pela distância.",
    figFrus_noteOrtho: "Afaste o B: ele mantém o tamanho — nada é dividido pela distância.",
    figHom_title: "Por que w? Translação como cisalhamento",
    figHom_s0: "Uma translação move a origem — nenhuma matriz consegue isso em 2D, porque M·(0,0) é sempre (0,0).",
    figHom_s1: "Adicione uma terceira coordenada w = 1: todo ponto (x, y) vira (x, y, 1). O plano inteiro agora flutua na altura 1.",
    figHom_s2: "Aplique uma matriz 3×3 com (tx, ty) na última coluna. Em 3D isso é um cisalhamento — a origem não sai do lugar — mas o plano w = 1 desliza exatamente (tx, ty).",
    figHom_s3: "Descarte o w e leia x, y: a forma foi transladada. A direção no piso w = 0 nunca se moveu.",
    figHom_rowNote: "Linha 1: x + tx·w. Com w = 1 você obtém x + tx; com w = 0 (uma direção) você obtém x — sem mudança.",
    ...
    figJ_clip: "Clip → NDC",
    figJ_screen: "Tela",
    figJ_c0: "Espaço local: o modelo como o artista o criou, centrado na própria origem. O ponto laranja é o vértice que vamos acompanhar.",
    figJ_c1: "A matriz de modelo o rotaciona e o leva para o seu lugar no mundo, perto da câmera.",
    figJ_c2: "A matriz de visão move o mundo inteiro para que a câmera fique na origem olhando para −Z.",
    figJ_c3: "A matriz de projeção mais a divisão por w espremem o frustum no cubo NDC: o que está longe fica menor, a pirâmide vira uma caixa. (O NDC é mão esquerda: agora +z aponta para longe.)",
    figJ_c4: "O viewport achata o cubo na janela: x e y viram pixels, z vai para o depth buffer.",
    figVec_title: "Dot e Cross — o que eles medem",
    ...
    figVec_neg: "Negativo",
    figVec_zero: "Zero",
    figVec_dotLight: "Normalize os dois e o dot vira só cos θ: é exatamente assim que a iluminação difusa mede o quanto uma superfície está virada para a luz.",
    figVec_dotHint: "arraste as pontas de a e b",
    figVec_crossNote: "Trocar a ordem inverte o resultado — é a regra da mão direita. Com θ = 0° ou 180° a área é zero e o produto vetorial também; em 90° ele é máximo. Para um triângulo, cross(B − A, C − A) é a normal da face.",
    figPhong_title: "Iluminação Phong — os vetores por trás do brilho",
    figPhong_behind: "luz abaixo da superfície: dot(N, L) < 0 → sem difusa, sem especular",
    figPhong_sphere: "A mesma luz numa esfera, vista de frente. Cada pixel roda as três fórmulas com a sua própria normal.",
    figPhong_note: "Arraste o sol e o olho. A difusa só se importa com θ (luz vs normal); a especular só com α (reflexão vs olho). Aumente o shininess e o brilho fica menor e mais nítido.",
    figPhong_blinnNote: "O Blinn-Phong compara N com o vetor intermediário H em vez de V com R. O brilho fica mais largo para o mesmo shininess, então projetos reais aumentam o expoente (algo como ×2–4).",
    figWind_title: "Ordem de winding — o mesmo triângulo, dos dois lados",
    figWind_hint: "gire e veja a ordem inverter",
    figWind_edge: "de lado: área zero, nada para desenhar",
    figWind_onScreen: "na tela",
    figWind_top: "vista de cima",
    figWind_rotate: "girar Y",
    figWind_note: "Os vértices nunca mudam — só o lugar de onde você olha. Depois de 90° o triângulo mostra as costas, a ordem na tela vira horária e, com back-face culling, ele some. Numa malha fechada essas faces de trás ficam escondidas de qualquer jeito, então o culling corta mais ou menos metade do trabalho.",
    figTex_title: "Amostrando uma textura",
    figTex_tabWrap: "Modos de wrap",
    figTex_tabMag: "Magnificação",
    figTex_tabMin: "Minificação e mipmaps",
    figTex_repeat: "Só a parte fracionária é usada: a imagem se repete para sempre. É o padrão.",
    figTex_mirror: "Repete, mas cada cópia alternada é espelhada, então as bordas sempre combinam com a vizinha.",
    figTex_clamp: "As coordenadas são limitadas a [0, 1]: os texels da borda se esticam para fora. Use em UI e em tudo que não pode vazar.",
    figTex_border: "Fora de [0, 1] você recebe uma cor de borda fixa (roxo aqui), definida com GL_TEXTURE_BORDER_COLOR.",
    figTex_wrapNote: "O quadrado branco é o intervalo [0, 1] da própria textura. Tudo fora dele é decidido pelo modo de wrap. Repare que o F aparece em pé: V cresce para cima, porque a origem da textura no OpenGL é o canto inferior esquerdo.",
    figTex_result: "resultado",
    figTex_linearNote: "O LINEAR mistura os quatro centros de texel mais próximos, com peso pela proximidade da amostra a cada um (o tamanho do ponto). Suave, mas borrado de perto — errado para pixel art.",
    figTex_nearestNote: "O NEAREST pega o único texel em que a amostra cai. Blocos nítidos — perfeito para pixel art, duro para fotos.",
    figTex_dragHint: "arraste o ponto de amostra laranja",
    figTex_tint: "colorir níveis de mip",
    figTex_minNearest: "Ao longe, um pixel da tela cobre muitos texels mas só um é escolhido: o xadrez vira ruído e moiré, e cintila quando a câmera se move.",
    figTex_minMip: "glGenerateMipmap cria cópias com metade do tamanho da textura, cada uma já pré-calculada pela média. A GPU escolhe o nível cujos texels batem com o tamanho do pixel, então a distância vira um cinza suave em vez de ruído.",
    figTex_minTint: "Cada cor é um nível de mip: vermelho é a textura em tamanho cheio, e cada nível seguinte tem metade do tamanho do anterior. Os pixels distantes leem de níveis minúsculos, já pré-calculados.",
    ...
    ch08la_basisBody:    "Antes das fórmulas 4×4, uma ideia torna todas elas legíveis: uma matriz só registra para onde vão os vetores da base. Em 2D, î = (1, 0) e ĵ = (0, 1). A primeira coluna da matriz é o novo î, a segunda coluna é o novo ĵ. Todo outro ponto é construído a partir desses dois — o vetor (x, y) é x passos de î mais y passos de ĵ, então M · (x, y) = x·î + y·ĵ.",
    ch08la_basisTry:     "Arraste o î vermelho e o ĵ verde abaixo e veja a grade inteira acompanhar. Os números de cada coluna da matriz são só as coordenadas da seta da mesma cor.",
    ch08la_basisTip:     "O quadrado sombreado é o quadrado 1×1 original depois da transformação. A área dele é o determinante: det = 2 significa áreas dobradas, det < 0 significa que o espaço foi espelhado (o F fica ao contrário), e det = 0 significa que tudo foi achatado numa linha — informação foi perdida, e é por isso que essa matriz não tem inversa. Em 3D é igual, com uma terceira coluna para k̂.",
    calloutNote:    "NOTA",
    ...
    ch08_orderBody:   "Por que a ordem importa tanto? Porque rotação e escala sempre acontecem em torno da origem (0, 0, 0). Dê play na figura abaixo: as mesmas duas operações, em ordens opostas, dão resultados bem diferentes.",
    ch08_orderCode:   "Cada chamada do glm multiplica a nova matriz pela direita: model = model × M. Então a chamada que você escreve por último é a primeira a tocar o vértice. Para escalar, depois rotacionar, depois transladar, escreva na ordem oposta:",
